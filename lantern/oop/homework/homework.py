class Cat:
  ...
  * Implement get_average_speed and return average_speed
  """
    ...
    def run(self, hours):
        running = self.average_speed * hours
        if running <= 25:
  ...
  * Implement method number_of_rolls_of_wallpaper which receives such parameters: roll_width_m, roll_length_m
  ...
  """
    ...
    def number_of_rolls_of_wallpaper(self, roll_width_m, roll_length_m):
        ...
        return count_of_lines / count_lines_in_roll
  ...
  * Implement method get_room_square that returns the square of our room
  ...
  """
    ...
    def get_walls_square(self):
        sum_walls_square = 0
        for _ in self.__walls:
            sum_walls_square += Wall.wall_square(_)
        return sum_walls_square

    def get_windows_square(self):
        sum_window_square = 0
        for _ in self.__windows:
            sum_window_square += Window.window_square(_)
        return sum_window_square
    ...
    def get_number_of_rolls_of_wallpapers(self, roll_width_m, roll_length_m):
        sum_of_the_number_of_rolls = 0
        if roll_width_m == 0 or roll_length_m == 0:
            raise ValueError("Sorry length must be not 0")
        else:
            for _ in self.__walls:
                sum_of_the_number_of_rolls += Wall.number_of_rolls_of_wallpaper(_, roll_width_m, roll_length_m)
            return sum_of_the_number_of_rolls
